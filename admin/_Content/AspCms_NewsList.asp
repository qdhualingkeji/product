...
<div class="right_title"><strong class="txt_C3">内容列表</strong></div>
<div class="main_center_article" id="web_main">
	<div class="main_form"  style=" background:#e8f1f6">
        <div class="main_form_news_l"><strong><%=styleName%>列表</strong> | <a href="AspCms_NewsAdd.asp?pic=<%=pic%>&sort=<%=sortID%>"  class="txt_C1">发布<%=styleName%></a></div>
    </div>
    <div class="maintable">    
	<form action="?pic=<%=pic%>&page=<%=page%>&order=<%=order%>&sort=<%=sortID%>&keyword=<%=keyword%>" method="post" name="form">
        <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#a9c5d0" style=" margin:5px 0px;">
            <tr bgcolor="#FFFFFF" align="center">
                <td style=" height:30px;">关键字：<input type="text" name="keyword" value="<%=keyword%>" style="width:200px;" />&nbsp;&nbsp;<input type="submit" name="selectBtn" value="搜索" class="btn" />        <input type="button" name="selectBtn" value="全部" class="btn"  onclick="location.href='?pic=<%=pic%>&sort=<%=sortID%>'" /></td>
            </tr>
        </table>         
