...
<body>
  ...
  <div class="main_center_article" id="web_main">
	  <form action="?action=addproduct" method="post" name="form">
	  	<div class="main_form"  style=" background:#e8f1f6">
	  	  <div class="main_form_news_l"><strong>添加产品</strong> | <a href="AspCms_productList.asp?sort=<%=SortID%>" class="txt_C1">查看产品列表</a></div>
	  	  </div>
		...
		<div class="main_form_input"> 
        	<input type="text" style="width:300px;" name="Title" maxlength="200"/>
            <input type="hidden" name="SortID" value="<%=getForm("sort","get")%>" />
		</div>
		</div>
        <%Call getSpec(0)%>        
  ...
  <%'Dim oFCKeditor:Set oFCKeditor = New FCKeditor:oFCKeditor.BasePath="../fckeditor/":oFCKeditor.ToolbarSet="aspcms":oFCKeditor.Width="615":oFCKeditor.Height="300":oFCKeditor.Value=decodeHtml(Content):oFCKeditor.Create "Content"%>  
		...
		<div class="main_form_input"><input type="checkbox" name="NewsStatus" value="1" checked="checked" />         
		</div>
		</div>
