...
<div class="main_form"  style=" background:#e8f1f6">
  <div class="main_form_news_l"><strong>模板管理</strong>  <%=defaultTemplate%>/html文件夹</div>
</div>
...
Sub htmlFileList
	...
	path = "/"&sitePath&"Templates/"&defaultTemplate&"/"&htmlFilePath	
	...
	if instr(fileListArray(0),",")>0 then	
		for  i = 0 to ubound(fileListArray)
			...
			"<td><a href=""AspCms_TemplateEdit.asp?filename="&fileAttr(0)&""" class=""txt_C1"" >编辑</a> | <a href=""?action=delallfile&filename="&fileAttr(0)&""" class=""txt_C1"" onClick=""return confirm('确定要删除吗')"">删除</a></td>"&vbcrlf& _
	...
	if not instr(fileListArray(0),",")>0 then
		...
		"<td colspan=""7""><a href=""javascript:location.href='AspCms_TemplateAdd.asp'""  class=""txt_C1"">新建模板<a/> <input type=""submit"" value=""批量删除"" onClick=""if(confirm('确定要删除吗')){return true;}else{return false}"" class=""btn""  />全选<input type=""checkbox"" name=""chkall"" id=""chkall"" class=""checkbox"" onClick=""checkAll(this.checked,'input','filename')"" />反选<input type=""checkbox"" name=""chkothers"" id=""chkothers"" class=""checkbox"" onClick=""checkOthers('input','filename')"" />&nbsp;&nbsp;</td></tr>"
	end if
End Sub
